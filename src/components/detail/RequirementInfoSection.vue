<template>
  <el-card shadow="never" class="info-card">
    ...
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="状态">{{ getStatusText(requirement?.status || '') }}</el-descriptions-item>
      <el-descriptions-item label="优先级">{{ getPriorityText(requirement?.priority || '') }}</el-descriptions-item>
      <el-descriptions-item label="创建时间">{{ formatDate(requirement?.created_at) }}</el-descriptions-item>
      <el-descriptions-item label="更新时间" :span="2">{{ formatDate(requirement?.updated_at) }}</el-descriptions-item>
      <el-descriptions-item label="描述" :span="2">
        <div class="description-content">{{ requirement?.description || '暂无描述' }}</div>
